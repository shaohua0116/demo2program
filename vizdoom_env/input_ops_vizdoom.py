...
def create_input_ops(dataset,
    ...
    with tf.device("/cpu:0"), tf.name_scope(scope):
        ...
        p, pt, s, ts, a, at, ta, tat, pl, dl, tdl, per, tper, \
            ip, ipl, tip, tipl = dataset.get_data(data_id[0])
        ...
        def load_fn(id):
            ...
            program, program_tokens, s_h, test_s_h, a_h, a_h_tokens, \
                ...
                per, test_per, init_pos, init_pos_len, \
                test_init_pos, test_init_pos_len= dataset.get_data(id)
            return (id, program.astype(np.float32), program_tokens.astype(np.int32),
                    ...
                    init_pos.astype(np.int32), init_pos_len.astype(np.int32),
                    test_init_pos.astype(np.int32), test_init_pos_len.astype(np.int32))
        ...
        input_ops['id'], input_ops['program'], input_ops['program_tokens'], \
            ...
            input_ops['test_init_pos'], input_ops['test_init_pos_len'] = tf.py_func(
        ...
        input_ops['init_pos'].set_shape(list(ip.shape))
        input_ops['init_pos_len'].set_shape(list(ipl.shape))
        input_ops['test_init_pos'].set_shape(list(tip.shape))
        input_ops['test_init_pos_len'].set_shape(list(tipl.shape))
